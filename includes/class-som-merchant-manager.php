...
class SOM_Merchant_Manager {
	...
	public static function create_merchant( $username, $password, $email = '', $shop_id = 0 ) {
		if ( empty( $username ) || empty( $password ) ) {
			return new WP_Error( 'missing_fields', __( 'Username and password are required.', 'shop-onboarding-manager' ) );
		}

		if ( username_exists( $username ) ) {
			return new WP_Error( 'username_exists', __( 'A merchant with this username already exists.', 'shop-onboarding-manager' ) );
		}

		if ( ! empty( $email ) && email_exists( $email ) ) {
			return new WP_Error( 'email_exists', __( 'A merchant with this email address already exists.', 'shop-onboarding-manager' ) );
		}
	...
	public static function ajax_merchant_login() {
		...
		if ( empty( $username ) || empty( $password ) ) {
			wp_send_json_error( array( 'message' => __( 'Please enter both username and password.', 'shop-onboarding-manager' ) ) );
		}
		...
		if ( is_wp_error( $user ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid username or password.', 'shop-onboarding-manager' ) ) );
		}

		// Verify user is a merchant or admin.
		if ( ! self::is_user_merchant( $user->ID ) && ! in_array( 'administrator', (array) $user->roles, true ) ) {
			wp_logout();
			wp_send_json_error( array( 'message' => __( 'Account is not registered as a Merchant.', 'shop-onboarding-manager' ) ) );
		}

		$shop_id = get_user_meta( $user->ID, 'som_shop_id', true );
		$shop_name = $shop_id ? get_the_title( $shop_id ) : __( 'Unassigned', 'shop-onboarding-manager' );

		wp_send_json_success(
			array(
				'message'      => __( 'Login successful!', 'shop-onboarding-manager' ),
	...
	public static function ajax_create_merchant_account() {
		...
		wp_send_json_success(
			array(
				'message' => __( 'Merchant account created successfully!', 'shop-onboarding-manager' ),
				'user_id' => $result,
			)
		);
	}
	...
	public static function render_login_shortcode() {
		...
		if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['log'], $_POST['pwd'], $_POST['som_m_login_submit'] ) ) {
			...
			if ( ! empty( $username ) && ! empty( $password ) ) {
				...
				if ( is_wp_error( $user ) ) {
					$server_msg = __( 'Invalid username or password.', 'shop-onboarding-manager' );
					$server_error = true;
				} elseif ( ! self::is_user_merchant( $user->ID ) && ! in_array( 'administrator', (array) $user->roles, true ) ) {
					wp_logout();
					$server_msg = __( 'Account is not registered as a Merchant.', 'shop-onboarding-manager' );
					$server_error = true;
				} else {
					wp_safe_redirect( home_url( '/merchant-dashboard/' ) );
					exit;
				...
				$server_msg = __( 'Please enter both username and password.', 'shop-onboarding-manager' );
				$server_error = true;
			}
		}
		...
		if ( is_user_logged_in() ) {
			...
			$shop_title = $shop_id ? get_the_title( $shop_id ) : __( 'No shop linked yet', 'shop-onboarding-manager' );
			...
			<div class="som-merchant-card">
				<div class="som-card-header">
					<h2><?php esc_html_e( 'Merchant Portal', 'shop-onboarding-manager' ); ?></h2>
					<p><?php printf( esc_html__( 'Logged in as %s', 'shop-onboarding-manager' ), '<strong>' . esc_html( $current_user->display_name ) . '</strong>' ); ?></p>
				</div>
				<div class="som-merchant-info">
					<div class="som-info-row">
						<span><?php esc_html_e( 'Linked Shop:', 'shop-onboarding-manager' ); ?></span>
						<strong><?php echo esc_html( $shop_title ); ?></strong>
					</div>
					<?php if ( $owner_name ) : ?>
					<div class="som-info-row">
						<span><?php esc_html_e( 'Owner:', 'shop-onboarding-manager' ); ?></span>
						<strong><?php echo esc_html( $owner_name ); ?></strong>
					</div>
					<?php endif; ?>
					<?php if ( $phone ) : ?>
					<div class="som-info-row">
						<span><?php esc_html_e( 'Phone:', 'shop-onboarding-manager' ); ?></span>
		...
		<div class="som-merchant-card">
			<div class="som-card-header">
				<h2>🔑 <?php esc_html_e( 'Merchant Login', 'shop-onboarding-manager' ); ?></h2>
				<p><?php esc_html_e( 'Access your shop onboarding portal', 'shop-onboarding-manager' ); ?></p>
			</div>
			...
			<form id="som_merchant_login_form" method="post" action="">
				...
				<div class="som-form-group">
					<label for="som_m_username" class="som-label"><?php esc_html_e( 'Username', 'shop-onboarding-manager' ); ?></label>
					<input type="text" id="som_m_username" name="log" class="som-input" required placeholder="<?php esc_attr_e( 'Enter username', 'shop-onboarding-manager' ); ?>" />
				</div>

				<div class="som-form-group">
					<label for="som_m_password" class="som-label"><?php esc_html_e( 'Password', 'shop-onboarding-manager' ); ?></label>
					<input type="password" id="som_m_password" name="pwd" class="som-input" required placeholder="<?php esc_attr_e( 'Enter password', 'shop-onboarding-manager' ); ?>" />
				</div>

				<div class="som-form-actions">
					<button type="submit" id="som_btn_merchant_login" class="som-submit-btn">
						<?php esc_html_e( 'Log In', 'shop-onboarding-manager' ); ?>
					</button>
				</div>
